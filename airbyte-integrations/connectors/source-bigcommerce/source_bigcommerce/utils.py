...
class EagerlyCachedStreamState:
    ...
    def stream_state_to_tmp(*args, state_object: Dict = cached_state, **kwargs) -> Dict:
        ...
        Because of the specific of Bigcommerce's entities relations, we have the opportunity to fetch the updates,
        for particular stream using the `Incremental Refresh`, inside slicing.
        For example:
            if `order_products` records were updated, then the `orders` is updated as well.
            etc.
        """
        # Map the input *args, the sequence should be always kept up to the input function
        ...
        # Save the current stream value for current sync, if present.
        if current_stream_state:
            state_object[stream.name] = {stream.cursor_field: current_stream_state.get(stream.cursor_field, "")}
            # Check if we have the saved state and keep the minimum value
